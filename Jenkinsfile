pipeline {
    ...
    environment {
        ...
        DOCKER_IMAGE_BACKEND = "${DOCKERHUB_NAMESPACE}/crud-spring"
        DOCKER_IMAGE_FRONTEND = "${DOCKERHUB_NAMESPACE}/crud-angular"
    }
    triggers {
        githubPush() 
    }
    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url: 'https://github.com/Shayoo-Sassi/TP-devops.git'
            }
        }
        stage('Build Backend') {
            steps {
